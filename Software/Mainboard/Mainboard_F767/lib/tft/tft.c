...
void LCD_Init(void)
{
	...
	LCD_Send_Dat(0x23);  // prescaler(M=29) 
	LCD_Send_Dat(0x02);  //multiplier(N=2) 
	LCD_Send_Dat(0x54);  //on-off multiplier and presceller
	//3. Turn on the PLL 
	LCD_Send_Cmd(0xE0);
	LCD_Send_Dat(0x01);
	HAL_Delay(120); // Wait for 100us to let the PLL stable 
	LCD_Send_Cmd(0xE0);
	...
	LCD_Send_Dat(0xDF);
	LCD_Send_Dat(0x00);     // 000 = режим RGB
	...
	LCD_Send_Dat(0x00);     //FPS: Vertical Display Period Start Position = 0 
	LCD_Send_Dat(0x08);
	...
	//if (MIRROR_H) LCD_Send_Dat(0x02); 
    //else if (MIRROR_V) LCD_Send_Dat(0x03);
    LCD_Send_Dat(0x22);
...
void LCD_Init(void)
{
	...
	//a. Set the panel size to 480 x 800 and polarity of LSHIFT, LLINE and LFRAME to active low 
	LCD_Send_Cmd(0xB0);
	//if (LSHIFT) LCD_Send_Dat(0x0C); 
    //else LCD_Send_Dat(0xAE); //18bit panel, disable dithering, LSHIFT: Data latch in rising edge, LLINE and LFRAME: active low 
	LCD_Send_Dat(0x20);  // TFT type 
	LCD_Send_Dat(0x00);
	LCD_Send_Dat(0x03);     // Horizontal Width:  480 - 1 = 0x031F 
	LCD_Send_Dat(0x1F);
	LCD_Send_Dat(0x01);     // Vertical Width :  800 -1 = 0x01DF
	LCD_Send_Dat(0xDF);
	LCD_Send_Dat(0x2D);  // 000 = режим RGB
											 //b. Set the horizontal period 
									 
	LCD_Send_Cmd(0xB4); // Horizontal Display Period  
	LCD_Send_Dat(0x04);    // HT: horizontal total period (display + non-display) – 1 = 520-1 =  519 =0x0207
	LCD_Send_Dat(0x20);
	LCD_Send_Dat(0x00);    // HPS: Horizontal Sync Pulse Start Position = Horizontal Pulse Width + Horizontal Back Porch = 16 = 0x10 
	LCD_Send_Dat(0x2E);
	LCD_Send_Dat(0x30);     // HPW: Horizontal Sync Pulse Width - 1=8-1=7 
	...
	LCD_Send_Dat(0xF0);     // PWM duty cycle  (50%) 
	LCD_Send_Dat(0x01);     
	//LCD_Send_Cmd(0x36);     // set address_mode
	//if (MIRROR_H) LCD_Send_Dat(0x02); 
    //else if (MIRROR_V) LCD_Send_Dat(0x03);
