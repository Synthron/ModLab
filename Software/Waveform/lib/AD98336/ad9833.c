 ...
 * 
 * @maintainer Synthron <admin@synthron.de>
 * Bugfix: frequency register value calculation
 * Cleanup: unecessary config write omitted
 * Modification: 
 *  -   changed to SPI1 on STM32F103 
 *  -   changed FSYNC-Pin name
 */
...
void AD9833_SetFrequency(uint32_t freq)
{
	...
	uint32_t freq_reg = (float)freq * (float)(((1 << 28)) / (float)FMCLK); // Tuning word
